...
public class ProfileService {
    ...
    @Transactional
    public ProfileResponse get(AppUser requestedUser) {
        AppUser user = managedUser(requestedUser.getId());
        ...
        // Repair older local-development records that already have a profile row but
        // were created before profile_completed was persisted correctly.
        if (!user.isProfileCompleted()) {
            user.markProfileCompleted(profile.getFullName());
            users.save(user);
        }
    ...
    @Transactional
    public ProfileResponse upsert(AppUser requestedUser, ProfileUpsertRequest request) {
        AppUser user = managedUser(requestedUser.getId());
    ...
    private AppUser managedUser(String userId) {
