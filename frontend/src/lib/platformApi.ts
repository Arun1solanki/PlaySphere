...
export const platformApi = {
  ...
  nearbyTurfs: (latitude: number, longitude: number, radiusKm = 15) =>
    apiRequest<NearbyTurf[]>(`/turfs/nearby?latitude=${encodeURIComponent(latitude)}&longitude=${encodeURIComponent(longitude)}&radiusKm=${encodeURIComponent(radiusKm)}`),
  searchLocations: (query: string) =>
    apiRequest<GeocodingResult[]>(`/locations/search?q=${encodeURIComponent(query)}`),
  reverseLocation: (latitude: number, longitude: number) =>
    apiRequest<GeocodingResult>(`/locations/reverse?latitude=${encodeURIComponent(latitude)}&longitude=${encodeURIComponent(longitude)}`),
  ...
  generateSlots: (id: string, body: unknown) => apiRequest<TurfSlot[]>(`/turfs/${id}/slots/generate`, { method: 'POST', body: JSON.stringify(body) }),
  ...
  generateFixtures: (id: string) => apiRequest<Match[]>(`/events/${id}/matches/generate`, { method: 'POST' }),
