import { type ChangeEvent, type PropsWithChildren, useEffect, useMemo, useState } from 'react'
...
export function RoleShell({ role, children }: RoleShellProps) {
  ...
  useEffect(() => {
    let cancelled = false
    void Promise.all([
      apiRequest<{ profileImageUrl?: string }>('/profile').catch(() => ({ profileImageUrl: '' })),
      apiRequest<Notification[]>('/notifications').catch(() => []),
      ...
      <main className="portal-main">
        <header className="portal-topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="icon-button" aria-label="Notifications" onClick={() => navigate(`/app/${theme.routeSegment}/notifications`)}>◌{unreadNotifications > 0 && <span className="notification-count">{unreadNotifications > 99 ? '99+' : unreadNotifications}</span>}</button>
            <button className="profile-chip" onClick={() => navigate('/profile/setup')}>
              <span className="profile-avatar">{profileImageUrl ? <img src={profileImageUrl} alt="Profile" /> : (user?.displayName ?? theme.label).charAt(0)}</span>
